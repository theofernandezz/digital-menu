...
**F1-4a (tables identified by number) — done, verified locally, and applied to the hosted project on 2026-09-24.** Decided while designing F1-4: a table is `table_number` (integer 1-999, unique per restaurant), not a free-text `label`, so the whole system reads one type (and numbers sort as numbers: `"10"` sorts before `"2"` as text). The UI renders it as "Mesa N". Built by `backend` from `specs/f1-4a-table-number.md`, checked by `verifier` (7 of 7 criteria PASS, frozen diff). `supabase/migrations/20260924000000_dining_tables_number.sql` drops `label` (and its unique constraint), adds `table_number` with the check and `unique (restaurant_id, table_number)`, and drops and recreates `get_table_status(text)` returning `(table_number, is_open)` with the same properties and grants as F1-2 (Postgres can't change a return type with `create or replace`, so the grants are re-applied). It fails on purpose if `dining_tables` has rows (nothing to backfill; production has none). The seed is now tables 1-3 with the same `dev-table-1..3` tokens. 135 integration tests pass twice in a row on one database (`createTable` numbers tables per restaurant from 1; `table-access` uses 900-903 in the seeded restaurant and cleans up).

- **Production script** (`prod-apply-f1-4a.sql`, not in the repo, like the first one): one transaction, preflight (right database, `label` present, `table_number` absent, `dining_tables` empty), the migration verbatim, assertions (column, both constraints, function return type, `security definer`, `stable`, empty `search_path`, exact ACL with PUBLIC none and only anon, authenticated, owner and `service_role` allowed, RLS and policy untouched). Tested locally against a simulated production state: applies; refuses a second run; refuses with a row present; rolls back on a failed assertion; plus 11 mutated versions of the script, each aborting with its own message and leaving the schema byte-identical. Applying it gives the same schema as running the migration.
- **Applied by the owner on 2026-09-24** (one paste in the SQL Editor, before merging the PR, like the first migrations). Its result on production matched the local database: `dining_tables` has `id`, `restaurant_id`, `qr_token`, `created_at` and `table_number integer not null` (no `label`); `get_table_status` returns `TABLE(table_number integer, is_open boolean)`; its ACL is the owner, `service_role`, `anon` and `authenticated`, nothing for `PUBLIC`. No data was touched (production has no tables).
- Found while checking: the first production script's `to_regproc('public.place_order(text, jsonb)')` check could never match (it needs `to_regprocedure`), so that preflight line was a no-op. Harmless (the other checks covered it and the script applied correctly); the new script uses `to_regprocedure`.
- Known limits: `table-access` relies on 900-903 being free in the seeded restaurant, and a crash before its `afterAll` would leave 900 behind until a `db reset`. The script's header says rows are "counted" but it only tests `exists`. The grantee whitelist would abort (safely) if the hosted project's default privileges gave `EXECUTE` to another role.
- **Open item, not decided: order history for the owner** (also the QR image and an automatic session close). Questions to settle after talking to someone who runs a venue: per-table session or a global list by date; read-only or correctable; how long it is kept. Nothing in F1 blocks it: orders keep name and price snapshots, closing a table keeps its sessions and orders, and the FK stops a table with orders from being deleted. Likely candidates for the next migration: this history (if it needs an index or RPC), Feature 2, an anon rate limit, and multi-tenant (composite FKs).

**F1-4b (`/admin/tables`) — done and verified locally (2026-09-24), not pushed yet.** Written while the owner was away, so the spec (`specs/f1-4b-admin-tables.md`) was **not reviewed by the owner beforehand**, by their explicit choice for this one; read it as a record of what was agreed with nobody but the verifier. Sequence: `backend` (domain to composition plus the actions), `ui` (page and components; it has no shell, so it could not run anything), `testing` (e2e and all gates), `verifier` twice (the first pass found a real client bug and two gaps; one correction round; the second pass is PASS on 7 of 7 criteria, with 7 and 3 mutations respectively all caught).

What it does: the owner opens `/admin/tables` (new "Mesas" nav entry and home card) and sees the tables ordered by number with "Abierta" or "Cerrada" and the absolute link `<origin>/t/<token>` (origin taken from the request headers, so the same page works on localhost and on Vercel); creates a table by number; opens and closes it. No migration: it reads and writes `dining_tables` and `table_sessions` directly under the owner's RLS. New: `DiningTable` entity, `table-errors.ts`, `DiningTableRepository` port, four use cases (`list-tables`, `create-table`, `open-table`, `close-table`), `SupabaseDiningTableRepository`, the actions and page in `app/admin/tables/`. 284 unit and 141 integration tests pass (twice in a row), plus `pnpm build` and the e2e (twice).

Decisions worth remembering:
- **`restaurant_id` comes from the table, never from the client.** `open-table` and `close-table` take only the table id, load it, assert ownership of the table's own restaurant and create the session with that id. This closes, at application level, the `TODO` in the F1-1 migration (the database still doesn't enforce it: composite FKs remain a multi-tenant item). A table the caller can't see (RLS) and one that doesn't exist both give `NotFoundError`, so nobody can probe another owner's tables.
- **Race-safe open:** the one-open-session unique index decides, and the action refreshes the page when it says `AlreadyOpenError`. **Close is idempotent.**
- **The QR token never reaches a client component**: only the ready-made link does.
- **Opening time not shown**: the model has no timezone, and a server-rendered clock time would be wrong on Vercel (UTC). Add a per-restaurant timezone before showing it.
- **`playwright.config.ts` now has `workers: 1`.** `fullyParallel: false` only serializes tests inside a file; with two spec files Playwright runs two workers, and `admin-lifecycle` signing out revokes that admin's sessions and killed the other spec's session, so the default `pnpm test:e2e` (and CI) failed intermittently. Any further e2e spec needs this.
- The new e2e picks a random table number (100-899) and deletes it afterwards through the service-role key, only if the Supabase URL is local.
- **Signing out revokes every session of that user** (supabase-js default), so signing out in one browser signs the owner out everywhere. Not changed; worth a decision (scope `local`) if that surprises anyone.
- Not done: `TableRowActions` has no test of its own (its error toast is only exercised through mocks), and the spec's UI checks lean on the e2e for the home card. A failed create clears the number field (React 19 resets forms after any action). Client components use `domain` constants (`MIN_TABLE_NUMBER`, `MAX_TABLE_NUMBER`), which pulls that small module into the client bundle. IPv6 hosts (`[::1]`) get `https` in the link.
- `AGENTS.md` shows a block that `next dev` writes into it ("This is NOT the Next.js you know"); it is not part of this work and was left out of the commits.
