...
- **One difference from local, found by that result:** production has a function `public.rls_auto_enable` that `anon` can execute (local has none). It is not ours; it looks like Supabase's "automatically enable RLS" helper (an event-trigger function). To confirm it cannot be called as an RPC, run in the SQL Editor: `select p.proname, p.prorettype::regtype as returns, p.prosecdef, p.proacl::text, (select string_agg(evtname || ' on ' || evtevent, ', ') from pg_event_trigger where evtfoid = p.oid) as event_trigger from pg_proc p where p.proname = 'rls_auto_enable';` — a return type of `event_trigger` used by an event trigger settles it.
