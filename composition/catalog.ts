// Catalog module: the restaurant, its categories, menu items and tags, and the
// public read model built from them. Wiring only — no logic lives here.
...
// The catalog module's public surface: this is what the rest of the app
// (app/) is allowed to call. Adding a capability means adding it here.
export type CatalogUseCases = {
  ...
  listMenuItems: ListMenuItemsUseCase;
  updateMenuItem: UpdateMenuItemUseCase;
  deleteMenuItem: DeleteMenuItemUseCase;
  getMyRestaurant: GetMyRestaurantUseCase;
  updateRestaurant: UpdateRestaurantUseCase;
  syncMenuItemTags: SyncMenuItemTagsUseCase;
  listMenuItemTags: ListMenuItemTagsUseCase;
  getPublishedMenu: GetPublishedMenuUseCase;
};

export function catalogUseCases(client: SupabaseClient): CatalogUseCases {
  return {
    createCategory: createCategoryUseCase(client),
    listCategories: listCategoriesUseCase(client),
    updateCategory: updateCategoryUseCase(client),
    deleteCategory: deleteCategoryUseCase(client),
    createMenuItem: createMenuItemUseCase(client),
    listMenuItems: listMenuItemsUseCase(client),
    updateMenuItem: updateMenuItemUseCase(client),
    deleteMenuItem: deleteMenuItemUseCase(client),
    getMyRestaurant: getMyRestaurantUseCase(client),
    updateRestaurant: updateRestaurantUseCase(client),
    syncMenuItemTags: syncMenuItemTagsUseCase(client),
    listMenuItemTags: listMenuItemTagsUseCase(client),
    getPublishedMenu: getPublishedMenuUseCase(client),
  };
}
